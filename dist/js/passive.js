...
export function SetGenerators() {
    if (basicGenerator)
        basicGenerator.textContent = `Generate Passive Points! | Generators: ${gameObjects.game.passiveGenerators[0].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[0].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[0].cost.toFixed(2)} Passive Points`;
    if (PPGenerator)
        PPGenerator.textContent = `Generate the First Generator | Generators: ${gameObjects.game.passiveGenerators[1].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[1].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[1].cost.toFixed(2)} 1st Generators`;
    if (PPGenerator2)
        PPGenerator2.textContent = `Generate the Second Generator | Generators: ${gameObjects.game.passiveGenerators[2].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[2].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[2].cost.toFixed(2)} 2nd Generators`;
    if (PPGenerator3)
        PPGenerator3.textContent = `Generate the Third Generator | Generators: ${gameObjects.game.passiveGenerators[3].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[3].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[3].cost.toFixed(2)} 3rd Generators`;
    if (PPGenerator4)
        PPGenerator4.textContent = `Generate the Fourth Generator | Generators: ${gameObjects.game.passiveGenerators[4].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[4].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[4].cost.toFixed(2)} 4th Generators`;
    if (PPGenerator5)
        PPGenerator5.textContent = `Generate the Fifth Generator | Generators: ${gameObjects.game.passiveGenerators[5].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[5].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[5].cost.toFixed(2)} 5th Generators`;
    if (PPGenerator6)
        PPGenerator6.textContent = `Generate the Sixth Generator | Generators: ${gameObjects.game.passiveGenerators[6].amountGained.toFixed(2)} (Bought: ${gameObjects.game.passiveGenerators[6].amountBought}) | Cost: ${gameObjects.game.passiveGenerators[6].cost.toFixed(2)} 6th Generators`;
    if (GeneratorButton)
        GeneratorButton.textContent = `Buy the next Tier of Generators! | Cost ${(gameObjects.game.passiveGenerators.findIndex((x) => x.amountGained == 0) +
            1) *
            3} Previous Tier Generators!`;
}
function BuyGenerator(generatorNumber) {
    if (generatorNumber == 0 &&
        ...
        gameObjects.game.passiveGenerators[generatorNumber].cost =
            gameObjects.game.passiveGenerators[generatorNumber].cost *
                Math.pow((gameObjects.game.passiveGenerators[generatorNumber].amountBought + 1), Math.log10(gameObjects.game.passiveGenerators[generatorNumber].amountBought + 1));
    }
    if (gameObjects.game.passiveGenerators[generatorNumber - 1].amountGained >=
        gameObjects.game.passiveGenerators[generatorNumber].amountGained) {
...
function BuyGeneratorTier() {
    var generatorToBuy = gameObjects.game.passiveGenerators.findIndex((x) => x.amountBought == 0);
    if (gameObjects.game.passiveGenerators[generatorToBuy - 1].amountGained >=
        ...
        const generator = document.querySelector(`#PassivePointsGenerator${generatorToBuy}`);
