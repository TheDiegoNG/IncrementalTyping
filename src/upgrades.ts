...
function ChangeBasicUpgradesText(upgradeNumber: number) {
    ...
    if(basicUpgradesText) basicUpgradesText.textContent = upgradesDesc[upgradeNumber];
}
...
function ChangePassiveUpgradesText(upgradeNumber: number) {
    ...
    if(passiveUpgradesText) passiveUpgradesText.textContent = passiveUpgradesDesc[upgradeNumber];
}
...
function ChangePrestigeUpgradesText(upgradeNumber: number) {
    ...
    if(prestigeUpgradesText) prestigeUpgradesText.textContent = prestigeUpgradesDesc[upgradeNumber];
}
...
function GetUpgrade(upgradeNumber: number, element: Element) {
    if(gameObjects.game.upgrades[upgradeNumber].amountBought == 0 && gameObjects.game.points >= gameObjects.game.upgrades[upgradeNumber].cost)
    {
        ...
        (element as HTMLElement).style.color = "#47682C";
        console.log(upgradeNumber);
        console.timeLog("GameTime")
    } 
}

function GetPassiveUpgrade(upgradeNumber: number, element: Element) {
...
function GetPrestigeUpgrade(upgradeNumber: number, element: Element) {
